...
void Zoom_out(GtkWindow*window)
{
	
	gchar* font = pluma_prefs_manager_get_editor_font();		//// take the curent font 
	char* pch;
	pch = strrchr(font, ' ');
	//printf("font: %s\n", font);
	//printf("last occurence found: %d\n", pch-font+1);
	char s[20];											////buffer
	int beginIndex = (int)(pch - font + 1);
	int endIndex = strlen(font);
	//printf("%d - %d\n", beginIndex, endIndex);
	strncpy(s, font + beginIndex, endIndex - beginIndex);
	s[endIndex - beginIndex] = '\0';
	//printf("Taille de la police: %s\n", s);
	int fontSize = (int)atoi(s) - 2;    			//// subtract 2 to the font 
	//printf("police: %d\n", fontSize);
	char buffer[20];
	sprintf(buffer,"%d",fontSize);
	int endIndex2 = strlen(buffer)+1;
	char newPolice [endIndex+1];
	strncpy(newPolice, font, beginIndex);
	//printf("new police: %s\n", newPolice);
	//printf("%d\n", (int) endIndex2);
	char buff[100];
	//printf("%s\n", buffer);
	snprintf(buff, sizeof buff, "%s%s",newPolice,buffer);
	//printf("new police: %s\n", buff);
	pluma_prefs_manager_set_editor_font(buff);

}

void Zoom_in(GtkWindow*window)
{
	...
	s[endIndex - beginIndex] = '\0';
	//printf("Taille de la police: %s\n", s);
	int fontSize = (int)atoi(s) + 2;				//// add 2 to the font
	//printf("police: %d\n", fontSize);
	char buffer[20];
	sprintf(buffer,"%d",fontSize);
	int endIndex2 = strlen(buffer)+1;
...
///////////////////////////function zoom in & zoom out /////////////////////////////////////




GClosure* closure = g_cclosure_new(Zoom_in,0,0);

    // Set up the accelerator group.
    GtkAccelGroup* accel_group = gtk_accel_group_new();
    gtk_accel_group_connect(accel_group,
                            GDK_KEY_KP_Add,				// add bindings ctrl + "+"
                            GDK_CONTROL_MASK,
                            0,
                            closure);
    gtk_window_add_accel_group(GTK_WINDOW(window), accel_group);

GClosure* closure2 = g_cclosure_new(Zoom_out,0,0);

    // Set up the accelerator group.
    GtkAccelGroup* accel_group2 = gtk_accel_group_new();
    gtk_accel_group_connect(accel_group,
                            GDK_KEY_KP_Subtract,		// add bindings ctrl + "-"
                            GDK_CONTROL_MASK,
                            0,
                            closure2);
    gtk_window_add_accel_group(GTK_WINDOW(window), accel_group2);
